...
```
```{r}
# is there a more intuitive way to use spread and gather?
spread <- function(dt, formula, value) {
  data.table::dcast(dt, eval(substitute(formula)), value.var = paste0(substitute(value)))
}
spread(subset2, index + `vehicle model` + cyl + disp ~ vs, kpL)

spread1 <- function(dt, key, value) {
  key <- deparse(substitute(key))
  value <- deparse(substitute(value))
  SDcols <- paste0("`",colnames(dt),"`")
  SDcols <- SDcols[c(-grep(key, SDcols),-grep(value, SDcols))]
  SDcols <- paste(paste(SDcols, collapse="+"))
  SDcols
}

spread1(subset2, vs, kpL)
```
...
```{r}
dt |> ascend_rows(cyl, vs, am)
dt |> descend_rows(cyl, vs, am)

dt |> set_group(cyl) |> ascend_rows(vs, am, gear)
dt |> set_group(cyl) |> descend_rows(vs, am, gear)
```
